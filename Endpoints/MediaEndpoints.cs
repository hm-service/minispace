...
public static class MediaEndpoints
{
    ...
    private const int ThumbSmallHeight = 384;
    ...
    public static IEndpointRouteBuilder MapMediaEndpoints(this IEndpointRouteBuilder app, string mediaDir)
    {
            ...
            string key, AppDbContext db, ThumbDbContext thumbs, HttpContext ctx) =>
        {
            ...
            if (mode is not null && mode != ThumbModeSmall)
    ...
    private static async Task<(byte[] Bytes, bool Failed)> GetOrCreateThumbAsync(
        ThumbDbContext db, string originalPath, string sha256)
    {
        byte[] original;
        try
        {
            original = await File.ReadAllBytesAsync(originalPath);
        }
        catch
        {
            return ([], true);
        ...
        try
        {
            using (var probe = Image.Load(original))
            {
                origW = probe.Width;
                origH = probe.Height;
            }
        ...
        var scale = Math.Min(1.0, (double)ThumbSmallHeight / origH);
