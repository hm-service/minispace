    ...
    setup() {
        ...
        async function login() {
            ...
            try {
                ...
                if (!isAuthPage) {
                    location.replace(
                        "/auth?next=" + encodeURIComponent(pathname + location.search),
                    );
                }
            }
        }
        ...
        async function submitPost() {
            ...
            try {
                ...
                for (let i = 0; i < draft.files.length; i++) {
                    ...
                    let m;
                    try {
                        m = await Api.upload(draft.files[i], (p) => {
                            draft.statuses[i] = p;
                        });
                    } catch (e) {
        ...
        function removeComment(postId, commentId) {
            ...
            openConfirm(
                "删除评论",
                `删除后不可恢复要删除 「${snippet(c?.content)}」 吗？`,
                async () => {
                    try {
        ...
        function removePost(postId) {
            ...
            openConfirm(
                "删除动态",
                `删除后不可恢复。确认要删除 「${snippet(p?.textContent)}」 吗？`,
                async () => {
                    try {
